...
def count_files():
    DIR = input('Введите адресс директории в виде "C:\\Users\\Ангелина"')
...
def show_products():
    k = 0
    file = open('C:\\Users\\Ангелина\\laba3\\tyap.txt', 'r')
...
def change_quantity():
    ...
    num = input('>>')
    print('Введите, на сколько нужно увеличить:')
    qual = input('>>')
    if int(qual) > 0:
        for elem in n_list:
            if elem[0] == num:
    ...
    if i == 'y':
        f = open('C:\\Users\\Ангелина\\laba3\\tyap.txt',  'w')
        f.write(str(header) + '\n')
        for elem in n_list:
            f.write(str(elem) + '\n')
        print('Файл записан!')
